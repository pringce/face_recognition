...
def Global_Average_Pooling(net):
    ...
    # return tf.layers.average_pooling2d(inputs=net, pool_size=pool_size)
...
def block35(net, scale=1.0, activation_fn=tf.nn.relu, scope=None, reuse=None):
    ...
    with tf.variable_scope(scope, 'Block35', [net], reuse=reuse):
        ...
        channel = int(np.shape(up)[-1])
        up = Squeeze_excitation_layer(up, out_dim=channel, ratio=reduction_ratio, layer_name='SE_A')
        net += scale * up
        if activation_fn:
...
def block17(net, scale=1.0, activation_fn=tf.nn.relu, scope=None, reuse=None):
    ...
    with tf.variable_scope(scope, 'Block17', [net], reuse=reuse):
        ...
        up = slim.conv2d(mixed, net.get_shape()[3], 1, normalizer_fn=None,
                         activation_fn=None, scope='Conv2d_1x1')
        channel = int(np.shape(up)[-1])
        up = Squeeze_excitation_layer(up, out_dim=channel, ratio=reduction_ratio, layer_name='SE_B')
        net += scale * up
        if activation_fn:
            net = activation_fn(net)
    return net
...
def block8(net, scale=1.0, activation_fn=tf.nn.relu, scope=None, reuse=None):
    ...
    with tf.variable_scope(scope, 'Block8', [net], reuse=reuse):
        ...
        up = slim.conv2d(mixed, net.get_shape()[3], 1, normalizer_fn=None,
                         activation_fn=None, scope='Conv2d_1x1')
        channel = int(np.shape(up)[-1])
        up = Squeeze_excitation_layer(up, out_dim=channel, ratio=reduction_ratio, layer_name='SE_C')
        net += scale * up
        if activation_fn:
            net = activation_fn(net)
    return net
